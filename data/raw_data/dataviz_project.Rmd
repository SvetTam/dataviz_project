...
## knitr::opts_knit$set(root.dir = '/home/getting-started/dataviz_project')
```
...
pets <- read.csv("seattle_pet_licenses.csv", stringsAsFactors = FALSE)
```
...
# question 1 how many pet licenses are included in the dataset?
class(pets$license_number) # it's a list so use length on it

length(pets$license_number)

# question 2 how many unique pet names are included in the dataset (animal_s_name)

class(unique(pets$animal_s_name))  # it's a list so use length on it

length(unique(pets$animal_s_name))


# question 3 how many different species are included in this dataset (species)?
class(pets$species)

length(unique(pets$species))

# question 4 which species are not included in the dataset (dog, primate, cat, livestock)?
...
# question 5 Are there more dog or cat licenses?
...
pets %>%
  ...
  arrange(-n) %>%
  top_n(10) %>%
  kable(., caption = "The 10 Top Dog Names in Seattle")
...
## read in the data
steak <- read.csv("steak-risk-survey.csv", stringsAsFactors = FALSE) %>%
...
# Q 13 Which piece of information was NOT collected about the steak survey respondents?

# names(steak)

# Q 14 How many people responded “Yes” to the question “Do you eat steak?”

# names(steak)

# tabyl(steak, "Do.you.eat.steak.")

# Q 15 How many different (unique) responses were there to the question “How do you like your steak prepared?”
...
## wrangle the steak data setgroup_
pref <- steak %>% 
...
## generate the plot 
```{r}
## copy of the code chunk above with the function_name replaced
## generate the plot 
p <- ggplot(pref) +
